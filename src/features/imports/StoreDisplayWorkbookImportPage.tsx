...
import { isBlockingStoreDisplayWorkbookIssue, StoreDisplayWorkbookImportAdapter, toApplyStoreDisplayWorkbookImport, type StoreDisplayWorkbookImportResult, type StoreDisplayWorkbookSheetMappings } from "../../adapters/import/StoreDisplayWorkbookImportAdapter";
...
export function StoreDisplayWorkbookImportPage() {
  ...
  const counts = result ? {
    ...
    blockingIssues: result.issues.filter(isBlockingStoreDisplayWorkbookIssue).length,
  } : undefined;
  ...
  return <DataState loading={loading} error={error}><div className="space-y-5"><PageHeader eyebrow="Store workbook → campaign" title="Import OND store display workbook" description="One worksheet per store. Product resolution and physical display placement are reviewed independently." actions={<Link className="rounded border border-border px-3 py-2 text-sm font-semibold" to="/imports">Imports</Link>} />
    ...
    {result && counts && <><Card><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Import review</h2><p className="text-sm text-text-secondary">Sheets: {result.sheetNames.join(", ")}</p></div><div className="flex flex-wrap gap-2"><Badge tone="success">{counts.ready} ready</Badge><Badge tone="warning">{counts.pending} pending product</Badge><Badge tone="warning">{counts.unresolved} unresolved display</Badge><Badge tone="warning">{counts.invalid} invalid</Badge><Badge tone={counts.noteConflicts ? "error" : "neutral"}>{counts.noteConflicts} conflicting notes</Badge></div></div><p className="mt-3 text-sm text-text-secondary">{counts.inactive} exact inactive Product Master matches are retained for review. Raw <b>N</b> is never converted to shelf support or no-display.</p></Card>
      ...
      {result.issues.some((item) => item.code === "missing_store_display_headers") && <Card><h2 className="font-semibold text-error">Workbook format error</h2><ul className="mt-2 space-y-1 text-sm text-error">{result.issues.filter((item) => item.code === "missing_store_display_headers").map((item) => <li key={`${item.field}-${item.code}`}>{item.message}</li>)}</ul><p className="mt-2 text-xs text-text-muted">Use the required headers exactly: Vendor, Category, INV_NUM, Product, Display, Case QTY, Display Notes.</p></Card>}
      <Card><div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]"><Field label="Apply to shared campaign"><select aria-label="Campaign" className={inputClass} value={campaignId} onChange={(event) => setCampaignId(event.target.value)}><option value="">Choose a draft campaign…</option>{data.campaigns.filter((campaign) => campaign.status === "draft").map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.name}</option>)}</select></Field><Button disabled={!campaignId || busy || counts.blockingIssues > 0 || counts.temporaryMarkers > 0 || !result.rows.some((row) => row.store && row.status !== "invalid")} onClick={() => void apply()}>Apply store display plan</Button></div><p className="mt-2 text-xs text-text-muted">{counts.blockingIssues ? `${counts.blockingIssues} workbook error${counts.blockingIssues === 1 ? " blocks" : "s block"} Apply. Correct the workbook and re-import.` : counts.temporaryMarkers ? `${counts.temporaryMarkers} temporary N source marker${counts.temporaryMarkers === 1 ? " blocks" : "s block"} Apply. Replace or remove them in the workbook and re-import.` : "Invalid and unknown-store rows remain in the imported review record; applicable pending products are retained as campaign-only planning products."}</p></Card>
