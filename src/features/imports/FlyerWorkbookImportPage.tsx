...
function displaySummary(row: FlyerWorkbookReviewRow) {
  const codes = [...new Set(row.allocations.map((allocation) => allocation.displayLocalCode).filter(Boolean))];
  if (codes.length > 1) return `${codes.length} store-specific codes`;
  return codes[0] ?? row.displayLocalCode ?? (row.displayRequired ? "Required, unassigned" : "Shelf");
}
