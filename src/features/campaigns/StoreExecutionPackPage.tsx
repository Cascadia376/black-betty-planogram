...
export function StoreExecutionPackPage() {
  ...
  const map = useFloorMapReadiness(pack?.layout?.backgroundImageUrl);
  ...
  return <main className="execution-pack">
    <nav className="pack-controls"><Link to={`/campaigns/${campaignId}/review`}>Campaign Review</Link><div>{ond && executionMonths.map((item) => <Link key={item} to={packUrl(item)}>{monthLabel(item)} order plan</Link>)}<Link to={packUrl()}>Display plan only</Link></div><button disabled={map.status !== "ready"} onClick={() => window.print()}>Print / Save letter-size PDF</button>{map.status === "error" && <button onClick={map.retry}>Retry floor map</button>}</nav>
    {map.status === "loading" && <p role="status" className="pack-controls">Loading the floor map. Printing unlocks when the image is ready.</p>}
    ...
    <section className="pack-map-page"><h2>{ond ? "OND " : ""}display floor map</h2><ExecutionMap key={`${releaseId ?? "draft"}:${pack.layout?.backgroundImageUrl}:${map.attempt}`} data={packData} layout={pack.layout} storeName={pack.store.name} areas={pack.builds.map((item) => item.area)} available={map.status !== "missing" && map.status !== "error"} onFailed={map.failed} /></section>
    <section className="pack-group"><h2>Display setup instructions</h2>{!pack.builds.length && <p>No confirmed display builds are available for this store.</p>}{pack.builds.map((build) => <article className="pack-build" key={build.id}>
      ...
      <div className="pack-location"><p>Highlighted location: {build.code}</p><ExecutionMap key={`${pack.layout?.backgroundImageUrl}:${map.attempt}`} available={map.status !== "missing" && map.status !== "error"} onFailed={map.failed} data={packData} layout={pack.layout} storeName={pack.store.name} areas={[build.area]} /></div>
    </article>)}</section>
...
function ExecutionMap({ data, layout, storeName, areas, available, onFailed }: { data: PlatformSnapshot; layout?: StoreLayout; storeName: string; areas: DisplayArea[]; available: boolean; onFailed(): void }) {
  if (!layout?.backgroundImageUrl || !available) return <p role="alert">Floor map unavailable — do not use this pack for location instructions.</p>;
  ...
  return <svg role="img" aria-label={`${storeName} full floor map with ${areas.length} campaign displays highlighted`} viewBox={`0 0 1000 ${height}`}>
    <image href={layout.backgroundImageUrl} width="1000" height={height} onError={onFailed} />
