...
function DisplayNotesEditor({ assignmentId, initialNotes, hasConflict, onSave }: { assignmentId: string; initialNotes?: string; hasConflict?: boolean; onSave(notes: string): Promise<void> }) {
  ...
  return <div>{hasConflict && <p role="alert" className="text-sm text-warning">Conflicting imported display notes require review before release.</p>}<Field label="Display notes"><textarea aria-label={`Display notes for ${assignmentId}`} className={`${inputClass} min-h-20 py-2`} value={notes} onChange={(event) => setNotes(event.target.value)} /></Field><Button type="button" disabled={saving || notes === (initialNotes ?? "")} onClick={() => { setSaving(true); void onSave(notes.trim()).finally(() => setSaving(false)); }}>Save display notes</Button></div>;
}
