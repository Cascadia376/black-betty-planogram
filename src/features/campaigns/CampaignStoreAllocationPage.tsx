...
export function CampaignStoreAllocationPage() {
  ...
  const [view, setView] = useState<"display" | "store">("store");
  ...
  return <DataState loading={loading} error={error}>{!campaign || !data ? <EmptyState title="Campaign not found" message="Unable to open store allocation." /> : <div className="space-y-5">
    ...
    <div className="flex flex-wrap gap-2"><Button variant={view === "store" ? "primary" : "secondary"} onClick={() => setView("store")}>By store</Button><Button variant={view === "display" ? "primary" : "secondary"} onClick={() => setView("display")}>By display</Button><select aria-label="Allocation filter" className={inputClass} value={filter} onChange={(event) => setFilter(event.target.value)}><option value="attention">Needs attention</option><option value="all">All placements</option><option value="ready">Ready</option></select></div>
...
function DisplayAllocations({ displayId, campaignId, filter, onError }: { displayId: string; campaignId: string; filter: string; onError(message?: string): void }) {
  ...
  return <Card><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="font-semibold">{display.name}</h2><p className="text-sm text-text-secondary">{display.placementMode === "STANDARD" ? "Standard display — review each suggested store placement." : "Store-specific display — review only the stores that use it."}</p></div>{display.placementMode === "STANDARD" && <Button onClick={() => void runAllocationAction(() => platform.suggestCampaignDisplay({ campaignId, campaignDisplayId: displayId }), onError)}><Lightbulb className="h-4 w-4" />Suggest for all stores</Button>}</div><div className="mt-3 space-y-2">{rows.length ? rows.map((assignment) => assignment ? <AllocationRow key={assignment.id} assignmentId={assignment.id} onError={onError} /> : null) : display.placementMode === "STANDARD" ? scopes.filter((scope) => !data.campaignDisplayAssignments.some((item) => item.campaignDisplayId === displayId && item.storeId === scope.storeId)).map((scope) => <UnplannedRow key={scope.id} displayId={displayId} campaignId={campaignId} storeId={scope.storeId} onError={onError} />) : <p className="text-sm text-text-muted">This display is not used by an included store.</p>}</div></Card>;
}
...
function StoreAllocations({ storeId, campaignId, filter, onError }: { storeId: string; campaignId: string; filter: string; onError(message?: string): void }) {
  ...
  const visible = displays.flatMap((display) => { const allocation = data.campaignDisplayAssignments.find((item) => item.campaignDisplayId === display.id && item.storeId === storeId); return allocation && showAllocation(allocation.status, filter) ? [<AllocationRow key={allocation.id} assignmentId={allocation.id} onError={onError} />] : !allocation && display.placementMode === "STANDARD" && filter !== "ready" ? [<UnplannedRow key={display.id} displayId={display.id} campaignId={campaignId} storeId={storeId} onError={onError} />] : []; });
  if (!visible.length && filter === "attention") return null;
  return <Card><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">{store.name}</h2><Badge tone={filter === "attention" ? "warning" : "neutral"}>{visible.length} {filter === "attention" ? "exceptions" : "placements"}</Badge></div><div className="mt-3 space-y-2">{visible}</div></Card>;
}
...
function AllocationRow({ assignmentId, onError }: { assignmentId: string; onError(message?: string): void }) {
  ...
  const floorplan = `/stores/${store.id}/floorplan?campaign=${assignment.campaignId}&assignment=${assignment.id}${area ? `&area=${area.id}` : ""}`;
  return <div className="rounded border border-border p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><div><b>{store.name}</b> · {display.name}{assignment.intendedDisplayCode && <span className="ml-2"><Badge tone="info">Target {assignment.intendedDisplayCode}</Badge></span>}<span className="block text-text-muted">{area ? `${area.name} (${area.displayNumber})${area.active ? "" : " · inactive historical area"}` : assignment.suggestionDisplayAreaId ? `Suggested: ${data.displayAreas.find((item) => item.id === assignment.suggestionDisplayAreaId)?.name}` : "Choose a physical location or mark it optional"}</span>{assignment.suggestionReasons?.map((reason) => <span key={reason} className="block text-xs text-text-muted">• {reason}</span>)}</div><div className="flex flex-wrap gap-2"><Badge tone={assignment.status === "ASSIGNED" || assignment.status === "EXCLUDED" ? "success" : "warning"}>{humanize(assignment.status)}</Badge>{assignment.suggestionDisplayAreaId && assignment.status !== "ASSIGNED" && assignment.status !== "EXCLUDED" && <Button onClick={() => void update({ campaignDisplayAssignmentId: assignment.id, displayAreaId: assignment.suggestionDisplayAreaId, status: "ASSIGNED" })}>Accept suggestion</Button>}<Button variant="secondary" onClick={() => setExpanded(!expanded)}>{expanded ? "Close" : `${campaignTerminology.storePlacement} / quantities`}</Button><Link className="inline-flex min-h-9 items-center gap-1 rounded-md border border-border px-3 font-semibold" to={floorplan}><MapPin className="h-4 w-4" />Floorplan</Link></div></div>{expanded && <div className="mt-3 grid gap-3 border-t border-border pt-3"><Field label="Physical display area"><select className={inputClass} value={area?.active ? assignment.displayAreaId ?? "" : ""} onChange={(event) => { if (event.target.value) void update({ campaignDisplayAssignmentId: assignment.id, displayAreaId: event.target.value, status: "ASSIGNED", placementSource: "BUYER_SELECTED" }); }}><option value="">Choose active display area…</option>{candidates.map((candidate) => { const result = campaignDisplayAreaCompatibility(display, candidate, data); return <option key={candidate.id} value={candidate.id} disabled={result.status === "incompatible"}>{candidate.name} · {candidate.localCode ?? candidate.displayNumber} · {candidate.verificationStatus === "verified" ? "Verified" : "Needs verification"} · {humanize(result.status)}</option>; })}</select></Field><div className="flex flex-wrap gap-2">{area && <Button variant="secondary" onClick={() => void update({ campaignDisplayAssignmentId: assignment.id, displayAreaId: null, status: "UNASSIGNED" })}>Clear placement</Button>}<Button variant="secondary" onClick={() => void update({ campaignDisplayAssignmentId: assignment.id, status: "EXCLUDED", note: `Optional in ${store.name}: ${assignment.intendedDisplayCode ?? display.name} is not used.` })}>Optional / no display</Button>{area && <Link className="inline-flex min-h-9 items-center gap-1 rounded-md border border-border px-3 font-semibold" to={floorplan}><ExternalLink className="h-4 w-4" />View on floorplan</Link>}</div><DisplayNotesEditor assignmentId={assignment.id} initialNotes={assignment.executionNotes} hasConflict={assignment.hasConflictingExecutionNotes} onSave={(executionNotes) => update({ campaignDisplayAssignmentId: assignment.id, executionNotes, hasConflictingExecutionNotes: false })} /><QuantityEditor assignmentId={assignment.id} displayId={display.id} storeName={store.name} onError={onError} /></div>}</div>;
}

function DisplayNotesEditor({ assignmentId, initialNotes, hasConflict, onSave }: { assignmentId: string; initialNotes?: string; hasConflict?: boolean; onSave(notes: string): Promise<void> }) {
  const [notes, setNotes] = useState(initialNotes ?? ""); const [saving, setSaving] = useState(false);
  return <div>{hasConflict && <p role="alert" className="text-sm text-warning">Conflicting imported display notes require review before release. Save the reviewed notes to confirm them, including when keeping the existing wording.</p>}<Field label="Display notes"><textarea aria-label={`Display notes for ${assignmentId}`} className={`${inputClass} min-h-20 py-2`} value={notes} onChange={(event) => setNotes(event.target.value)} /></Field><Button type="button" disabled={saving || (!hasConflict && notes === (initialNotes ?? ""))} onClick={() => { setSaving(true); void onSave(notes.trim()).finally(() => setSaving(false)); }}>Save display notes</Button></div>;
}
