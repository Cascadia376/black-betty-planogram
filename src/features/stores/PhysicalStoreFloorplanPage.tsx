import { FloorplanNavigationGuard } from "./FloorplanNavigationGuard";
...
export function PhysicalStoreFloorplanPage() {
  ...
  const { data, loading, error, authEnabled, blackBettyRole, duplicateStoreLayout, setCurrentStoreLayout, updateCategorySpace, updateDisplayArea } = usePlatform();
  ...
  const [layoutEditState, setLayoutEditState] = useState<FloorplanEditState>({ dirty: false, saving: false });
  ...
  const layoutManagementMode = canManagePhysicalLayout && !selectedCampaign && !selectedProgram && params.get("mode") === "layout";
  const canEditDisplayPositions = layoutManagementMode && layout?.status === "current";
    ...
    <DataState loading={loading} error={error}>
      {!store ? <EmptyState title="Store not found" message="The requested store is not available." /> : !layout ? (
        ...
        <>
          ...
          {layoutManagementMode && !canEditDisplayPositions && <p className="rounded border p-3 text-sm">This is a {layout.status} category layout. Display positions belong to the store, not to individual category-layout versions. Select the current layout to move displays.</p>}
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-md border border-border bg-surface p-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {layoutManagementMode && <button type="button" disabled={saving || layoutEditState.dirty} onClick={duplicate} className="inline-flex min-h-9 items-center gap-2 rounded-md border border-border bg-surface px-3 text-sm font-semibold hover:bg-subtle disabled:opacity-50"><Copy className="h-4 w-4" />Duplicate as draft</button>}
              {layoutManagementMode && layout.status !== "current" && <button type="button" disabled={saving || layoutEditState.dirty} onClick={publish} className="inline-flex min-h-9 items-center rounded-md bg-primary px-3 text-sm font-semibold text-primary-foreground disabled:opacity-50">Make current</button>}
            </div>
          </div>
          ...
          <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
            <Card className="min-w-0 overflow-hidden p-0">
              ...
              <div className="p-4 sm:p-5">
                <FloorplanCanvas key={`${storeId}:${layout.id}:${layoutManagementMode}`} onEditStateChange={setLayoutEditState}
                onGeometrySave={canEditDisplayPositions ? async (edit) => { await updateDisplayArea({ displayAreaId: edit.areaId, expectedGeometry: edit.originalGeometry, patch: edit.sectionId ? {} : { geometry: edit.geometry }, sectionGeometry: edit.sectionId ? { sectionId: edit.sectionId, geometry: edit.geometry } : undefined }); } : undefined} storeName={store.name} zones={zones} fixtures={fixtures} areas={areas} displayAreaSections={displayAreaSections} categorySpaces={spaces} backgroundImageUrl={layout.backgroundImageUrl} backgroundAspectRatio={layout.backgroundAspectRatio} showBase={showBase} showCategories={showCategories} showDisplayAreas={showDisplayAreas} showCampaignPlacements={showCampaignPlacements} selectedAreaId={selectedArea?.id} selectedCategorySpaceId={selectedSpace?.id} stateFor={stateFor} onSelect={(id) => updateSelection("area", id)} onSelectCategorySpace={(id) => updateSelection("space", id)} />
                ...
                <Card className="overflow-hidden p-0 xl:sticky xl:top-24">
                  ...
                  {editing ? <CategorySpaceForm space={selectedSpace} saving={saving} onCancel={() => setEditing(false)} onSubmit={saveSpace} /> : (
                    <>
                      ...
                      {layoutManagementMode && <div className="border-t border-border p-4"><button type="button" onClick={() => setEditing(true)} className="inline-flex min-h-9 w-full items-center justify-center gap-2 rounded-md bg-primary px-3 text-sm font-semibold text-primary-foreground"><Edit3 className="h-4 w-4" />Edit category space</button></div>}
                    </>
                  )}
                </Card>
