...
export function StoreWorkspacePage() {
  ...
  return <DataState loading={loading} error={error}>{!store ? <EmptyState title="Store not found" message="The requested store is not available." /> : <><PageHeader eyebrow="Execute" title={`${store.name} merchandising workspace`} description="What needs my attention? Open a task, find its display area, and report issues from one operational view." actions={<><Link className="inline-flex min-h-9 items-center rounded-md border border-border bg-surface px-3 text-sm font-semibold" to={`/stores/${store.id}/orders`}>Review orders</Link><Link className="inline-flex min-h-9 items-center gap-2 rounded-md border border-border bg-surface px-3 text-sm font-semibold" to={`/stores/${store.id}/floorplan`}><MapPin className="h-4 w-4" />View floorplan</Link></>} />
    ...
    <Card className="overflow-hidden p-0"><div className="border-b border-border px-4 py-3"><h2 className="font-semibold">Display tasks</h2></div><div className="divide-y divide-border">{tasks.map((execution) => {
      ...
      const Icon = iconFor(execution.status);
      const requiredSkus = campaign?.products.filter((item) => item.required).map((item) => data?.products.find((product) => product.id === item.productId)?.sku ?? "Unresolved") ?? [];
      return <div key={execution.id} className="grid gap-3 px-4 py-4 md:grid-cols-[1fr_auto] md:items-center"><div className="flex gap-3"><span className={clsx("mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-md", execution.status === "completed" ? "bg-success/10 text-success" : execution.status === "issue" ? "bg-error/10 text-error" : "bg-warning/10 text-warning")}><Icon className="h-4 w-4" /></span><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold">{campaign?.name}</p><Badge tone={execution.status === "completed" ? "success" : execution.status === "issue" ? "error" : "warning"}>{humanize(execution.status)}</Badge></div><p className="mt-1 text-sm text-text-secondary">{area?.name} · Due {formatDate(execution.dueDate)}</p><p className="mt-1 text-xs text-text-muted">Required SKUs: {requiredSkus.join(", ")}</p></div></div><div className="flex gap-2"><Link className="inline-flex min-h-9 items-center rounded-md border border-border bg-surface px-3 text-sm font-semibold" to={`/stores/${store.id}/floorplan?area=${area?.id}`}>Locate</Link><Link className="inline-flex min-h-9 items-center gap-1 rounded-md bg-primary px-3 text-sm font-semibold text-primary-foreground" to={`/executions/${execution.id}`}>Open task <ArrowRight className="h-4 w-4" /></Link></div></div>;
    })}</div></Card>
  </>}</DataState>;
}
