...
import { OND_WORKSPACE_DATE, buildStoreWorkspaceModel } from "./storeWorkspaceModel";
...
function TaskList({ data, storeId, model }: { data: PlatformSnapshot; storeId: string; model: ReturnType<typeof buildStoreWorkspaceModel> }) {
  ...
  const resetTasks = model.assignments.filter((item) => item.resetRequired);
  ...
  return <section aria-labelledby="attention-tasks" className="min-w-0"><div className="mb-3 flex items-end justify-between gap-3"><div><h2 id="attention-tasks" className="text-base font-semibold">Attention queue</h2><p className="mt-1 text-xs text-text-muted">Highest-priority execution and ordering work for this store.</p></div><Badge tone="warning">{executionTasks.length + resetTasks.length + orderTasks.length} open</Badge></div><div className="space-y-3">
    {executionTasks.map((execution) => {
      ...
      return <TaskCard key={execution.id} type="EXECUTION" tone={execution.status === "issue" ? "error" : "warning"} icon={<Clock3 className="h-4 w-4" />} title={campaign?.name ?? "Display execution"} meta={`Display ${area?.displayNumber ?? "-"} · Due ${formatDate(execution.dueDate)}`} detail={execution.issue ?? `${humanize(execution.status)} · Complete the display and submit evidence.`} actions={<><Link className={linkClass} to={`/executions/${execution.id}`}>Open task <ArrowRight className="h-4 w-4" /></Link><Link className={linkClass} to={`/stores/${storeId}/floorplan?area=${area?.id}`}><MapPin className="h-4 w-4" />Locate display</Link><Link className={linkClass} to={`/executions/${execution.id}`}>Report issue</Link></>} />;
    })}
...
function OperationalTimeline({ data, storeId, programId, programEnd, majorDemandPhaseStart }: { data: PlatformSnapshot; storeId: string; programId?: string; programEnd?: string; majorDemandPhaseStart?: string }) {
  ...
  const deliveries = data.inboundOrders.filter((item) => item.storeId === storeId && item.expectedArrivalDate >= OND_WORKSPACE_DATE).map((item) => item.expectedArrivalDate).sort();
