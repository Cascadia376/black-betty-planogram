...
export function ExecutionPage() {
  ...
  return <DataState loading={platform.loading} error={platform.error}>{!context?.execution || !context.area || (!context.campaign && !context.displayAssignment)
...
function ExecutionForm({ context, completeExecution, canExecute }: { context: ExecutionContext; completeExecution: ReturnType<typeof usePlatform>["completeExecution"]; canExecute: boolean }) {
  ...
  const { execution, campaignAssignment, displayAssignment, area, program, period, products, title, requirement } = context;
  ...
  return <form onSubmit={submit} className="space-y-6">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_23rem]">
      <div className="space-y-4">
        ...
        <Card><h2 className="font-semibold">Display requirements</h2><dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2"><Detail label="Signage" value={requirement.signage} /><Detail label="Minimum merchandising requirement" value={requirement.minimumSpace} /><div className="sm:col-span-2"><Detail label="Required / recommended placement" value={requirement.executionNotes} /></div><Detail label="Assignment notes" value={displayAssignment?.notes ?? campaignAssignment?.notes ?? "No assignment notes"} /><Detail label="Store execution notes" value={campaignAssignment?.notes ?? "Published OND assignment"} /></dl></Card>
      </div>
      <div className="space-y-4">
