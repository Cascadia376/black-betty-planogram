...
export function readEnvironment(source: Record<string, unknown> = import.meta.env): AppEnvironment {
  ...
  if (result.data.VITE_DATA_ADAPTER === "supabase" && (!result.data.VITE_SUPABASE_URL || !(result.data.VITE_SUPABASE_PUBLISHABLE_KEY || result.data.VITE_SUPABASE_ANON_KEY))) {
    throw new Error("VITE_SUPABASE_URL and a Supabase publishable key are required for the Supabase adapter.");
  }
  return result.data;
}
